...
@Service public class AiService {
  ...
  public Map<String,String> translate(String id,String language){News n=news.get(id);return translateContent(n.getTitle(),n.getSummary(),n.getContent(),language);}
  // Translate arbitrary article text (works for local/mock articles that aren't stored in Mongo).
  public Map<String,String> translateContent(String title,String summary,String content,String language){Map<String,String> m=new HashMap<>();if(apiKey.isEmpty()||language==null||language.equalsIgnoreCase("hi")){m.put("title",title);m.put("summary",summary);m.put("content",content);return m;}try{Map<String,String> src=new HashMap<>();src.put("title",title==null?"":title);src.put("summary",summary==null?"":summary);src.put("content",content==null?"":content);String raw=generate("Translate the values to "+language+". Respond with ONLY a JSON object with keys title, summary, content and no other text, no preamble, no explanation:\n"+json.writeValueAsString(src));int b=raw.indexOf('{'),e2=raw.lastIndexOf('}');if(b>=0&&e2>b)raw=raw.substring(b,e2+1);JsonNode root=json.readTree(raw);m.put("title",root.path("title").asText());m.put("summary",root.path("summary").asText());m.put("content",root.path("content").asText());return m;}catch(Exception e){log.warn("LLM translate failed: {}",e.toString());throw new IllegalStateException("Translation service unavailable");}}
