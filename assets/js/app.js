...
async function setLanguage(lang){if(!LANGS.includes(lang))return;state.language=lang;localStorage.setItem('nn_language',lang);applyStaticI18n();renderNav();renderAuth();if(lang!=='hi'&&sourceArticles.length)toast(t('translating'));await localizeArticles();route()}
// Translate every loaded article's title/summary/content into the active language (cached), so the
// whole page — headlines, cards and article bodies — switches language, not just the static UI chrome.
async function localizeArticles(){const lang=state.language;if(lang==='hi'||!sourceArticles.length){articles=sourceArticles.length?sourceArticles:articles;return}articles=await Promise.all(sourceArticles.map(async a=>{const k=lang+':'+a.id;if(!artCache[k]){let r;try{r=await API.translateContent({title:a.title,summary:a.summary,content:a.content,language:lang});if(needsLocalTranslation(r,lang))r=localTranslation(a,lang)}catch{r=localTranslation(a,lang)}artCache[k]={title:r.title||a.title,summary:r.summary||a.summary,content:r.content||a.content}}return {...a,...artCache[k]}}))}
...
const API={
  ...
  createNews(body){return this.request('/news',{method:'POST',headers:adminHeaders(),body:JSON.stringify(body)})}, updateNews(id,body){return this.request(`/news/${id}`,{method:'PUT',headers:adminHeaders(),body:JSON.stringify(body)})}, deleteNews(id){return this.request(`/news/${id}`,{method:'DELETE',headers:adminHeaders()})},
...
function adminHeaders(){return {'Content-Type':'application/json','X-User-Email':state.user?state.user.email:''}}
const localArticles=[...ARTICLES];
let sourceArticles=[];// source-language (Hindi) articles as loaded from backend/mock
let articles=localArticles;// what's rendered — translated copies when a non-Hindi language is active
const artCache={};// `${lang}:${id}` -> {title,summary,content} translation cache
...
let adminList=[];
async function refreshArticles(){try{const remote=await API.news();sourceArticles=remote.length?mergeArticles(remote).map(normalize):localArticles.map(normalize);await localizeArticles()}catch{}}
...
(async function init(){
  ...
  sourceArticles=localArticles.map(normalize);
  await localizeArticles();
  route();
  try{
    const remote=await API.news();
    if(remote.length){sourceArticles=mergeArticles(remote).map(normalize)}
  }catch{
    toast(t('backendUnavailable'));
  }
  await localizeArticles();route();
})();
